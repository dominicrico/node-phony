...
phony.on('interfacesAdded', function(a) {
  return console.log('######## INTERFACES ADDED ###########');
});
...
phony.on('deviceFound', function(a) {
  if (a.device.Name.indexOf('Galaxy') !== -1) {
    return phony.selectDevice(a).then(function() {
      return phony.connectHandsfree().then(null, phony.createOBEXSession('pbap').then(function() {
        return phony.getPhoneBook();
      }), phony.createOBEXSession('map').then(function() {
        return phony.getMessages('inbox').then(function(msgs) {});
      }), console.log);
    });
  }
});
